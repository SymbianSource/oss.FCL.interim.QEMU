...
{
    if (dp) {
        /* VFP memory accesses only require word alignment, so split
           doubleword accesses.  */
        TCGv low = new_tmp();
        TCGv high = new_tmp();
        tcg_gen_qemu_ld32u(low, cpu_T[1], IS_USER(s));
        tcg_gen_addi_i32(high, cpu_T[1], 4);
        tcg_gen_qemu_ld32u(high, high, IS_USER(s));
        tcg_gen_concat_i32_i64(cpu_F0d, low, high);
        dead_tmp(high);
        dead_tmp(low);
    } else {
...
static inline void gen_vfp_st(DisasContext *s, int dp)
{
    if (dp) {
        ...
        TCGv val = new_tmp();
        TCGv addr = new_tmp();
        tcg_gen_trunc_i64_i32(val, cpu_F0d);
        tcg_gen_qemu_st32(val, cpu_T[1], IS_USER(s));
        tcg_gen_addi_i32(addr, cpu_T[1], 4);
        tcg_gen_shri_i64(cpu_F0d, cpu_F0d, 32);
        tcg_gen_trunc_i64_i32(val, cpu_F0d);
        tcg_gen_qemu_st32(val, addr, IS_USER(s));
        dead_tmp(addr);
        dead_tmp(val);
    } else {
...
static int disas_neon_ls_insn(CPUState * env, DisasContext *s, uint32_t insn)
{
    ...
    if (!vfp_enabled(env))
      return 1;
    /* FIXME: Honour alignment specifiers.  */
    VFP_DREG_D(rd, insn);
    rn = (insn >> 16) & 0xf;
    rm = insn & 0xf;
    load = (insn & (1 << 21)) != 0;
...
done_generating:
    ...
    *gen_opc_ptr = INDEX_op_end;
    ...
    if (loglevel & CPU_LOG_TB_IN_ASM) {
        ...
        target_disas(logfile, pc_start, dc->pc - pc_start,
